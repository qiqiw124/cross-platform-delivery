...
description: 跨前端、后端、客户端通用的需求交付技能。用于用户提供设计稿、需求文档、接口文档、埋点口径或业务规则并希望快速完成跨端对齐、联调、实现规划、memory维护、代码审查和验收清单时触发。适用于新功能、复杂重构、联调排障、字段口径不一致、状态机梳理、埋点补齐等场景。
---

# Cross Platform Delivery

## Overview
统一前端、后端、客户端的需求交付流程：先检查输入、再做跨端对齐；先复用已有逻辑、再新增实现；始终输出可联调、可审查、可回归的标准产物。
...
2. 优先检索已有逻辑与 memory。
- 默认先看 `memory.md`、`docs/memory/`、相似模块代码、已有接口模型、既有埋点实现。
...
- 需要快速起草交付物时，可运行 `scripts/init_delivery_artifacts.py --name <feature-name>` 生成 `mapping.md`、`memory.md`、`acceptance.md` 模板。

6. 强制维护业务 memory。
- 只要涉及业务规则、状态机、缓存、埋点、页面生命周期、前后台行为，就更新对应 memory。
- 多仓库模式下，优先拆成两层：
- `shared memory`：三端共用的业务口径、字段定义、埋点来源、验收规则。
- `repo memory`：各端自己的实现细节、生命周期、框架约束、代码路径。
- 可直接参考 `references/shared-memory-template.md` 起草 shared memory。
- 若没有现成 memory，新增业务 memory 文件并补充索引。
...
- 面向三端协作：用于统一需求口径、排查联调问题、沉淀 memory 和输出验收清单。
- 面向多仓库全栈：用于统一三个项目的字段口径、改动归属、shared memory 和跨仓库联调清单。

## Execution Rules
- 始终先对齐：需求、接口、埋点未对齐时，不直接编码。
- 始终先复用：先找已有 memory、模型、接口、组件、状态机。
- 始终可追踪：结论尽量附文件路径、接口名、字段名、页面名。
- 始终做映射表：跨端需求默认输出字段/状态/埋点映射。
- 始终做归属表：多仓库需求默认输出“字段归属 + 仓库归属 + 验收归属”。
- 始终维护 memory：有业务变更就更新。
- 始终做联调视角 review：不只看代码是否能跑。
- 若仓库或子模块已有更具体的本地 skill / memory / 目录规范，优先沿用本地规则，再用本 skill 做总控。
...
- `references/multi_repo_mode.md`：多仓库输入格式、仓库归属表、shared memory 建议
- `references/anti-patterns.md`：常见跨端协作反模式与纠偏方式
- `references/shared-memory-template.md`：多仓库共享业务 memory 模板
- `references/repo-memory-template.md`：单仓库实现 memory 模板
- `references/maintenance.md`：shared memory / repo memory 维护建议
- `examples/maintenance.md`：shared memory / repo memory 更新场景示例
- `examples/`：真实请求场景示例
